...
*/
(function () {
  const PLATFORM_VERSION_BUILD_LABEL = '1.0.0';
  ...
  (function () {
    ...
    require = function (id) {
      if (!modules[id]) {
        ...
        const cycle = `${requireStack
          .slice(inProgressModules[id])
          .join('->')}->${id}`;
  ...
  define('cordova', function (require, exports, module) {
    ...
    var cordova = {
      ...
      fireDocumentEvent(type, data, bNoDetach) {
        const evt = Capacitor.createEvent(type, data);
  ...
  define('cordova/argscheck', function (require, exports, module) {
    ...
    function checkArgs(spec, functionName, args, opt_callee) {
      ...
      if (errMsg) {
        ...
        errMsg = `Wrong type for parameter "${extractParamName(
          opt_callee || args.callee,
          i,
        )}" of ${functionName}: ${errMsg}`;
  ...
  define('cordova/base64', function (require, exports, module) {
    ...
    function uint8ToBase64(rawData) {
      const numBytes = rawData.byteLength;
      let output = '';
      let segment;
      const table = b64_12bitTable();
  ...
  define('cordova/builder', function (require, exports, module) {
    ...
    function include(parent, objects, clobber, merge) {
      each(objects, function (obj, key) {
        try {
          let result = obj.path ? require(obj.path) : {};
  ...
  define('cordova/channel', function (require, exports, module) {
    ...
    var channel = {
       ...
       */
      waitForInitialization(feature) {
        if (feature) {
          const c = channel[feature] || this.createSticky(feature);
       ...
       * @param feature {String}     The unique feature name
       */
      initializationComplete(feature) {
        const c = this.deviceReadyChannelsMap[feature];
        if (c) {
          c.fire();
        }
      },
    ...
    Channel.prototype.fire = function (e) {
      const fail = false; // eslint-disable-line no-unused-vars
      const fireArgs = Array.prototype.slice.call(arguments);
      // Apply stickiness.
      if (this.state === 1) {
        this.state = 2;
      ...
      if (this.numHandlers) {
        ...
        // callbacks.
        const toCall = [];
        for (const item in this.handlers) {
  ...
  define('cordova/exec', function (require, exports, module) {
    ...
    let execIframe;
    const commandQueue = []; // Contains pending JS->Native messages.
    const isInContextOfEvalJs = 0;
    ...
    function handleBridgeChange() {
      if (proxyChanged()) {
        let commandString = commandQueue.shift();
        while (commandString) {
          const command = JSON.parse(commandString);
          const callbackId = command[0];
          const service = command[1];
          const action = command[2];
          const actionArgs = command[3];
          const callbacks = cordova.callbacks[callbackId] || {};
    ...
    function cordovaExec() {
      const cexec = require('cordova/exec');
      const cexec_valid =
  ...
  define('cordova/exec/proxy', function (require, exports, module) {
    ...
    module.exports = {
      ...
      remove(id) {
        const proxy = CommandProxyMap[id];
  ...
  define('cordova/init', function (require, exports, module) {
    ...
    const platform = require('cordova/platform');
    const pluginloader = require('cordova/pluginloader');
    const utils = require('cordova/utils');

    const platformInitChannelsArray = [
    ...
    function replaceNavigator(origNavigator) {
      const CordovaNavigator = function () {};
      CordovaNavigator.prototype = origNavigator;
      const newNavigator = new CordovaNavigator();
  ...
  define('cordova/modulemapper', function (require, exports, module) {
    const builder = require('cordova/builder');
    const { moduleMap } = define;
    let symbolList, deprecationMap;
    ...
    exports.mapModules = function (context) {
      ...
      for (let i = 0, len = symbolList.length; i < len; i += 3) {
        const strategy = symbolList[i];
        const moduleName = symbolList[i + 1];
        const module = require(moduleName);
        // <runs/>
        if (strategy === 'r') {
          continue;
        }
        const symbolPath = symbolList[i + 2];
        const lastDot = symbolPath.lastIndexOf('.');
        const namespace = symbolPath.substr(0, lastDot);
        const lastName = symbolPath.substr(lastDot + 1);

        var deprecationMsg =
          symbolPath in deprecationMap
            ? `Access made to deprecated symbol: ${symbolPath}. ${deprecationMsg}`
            : null;
        const parentObj = prepareNamespace(namespace, context);
        const target = parentObj[lastName];
    ...
    exports.getOriginalSymbol = function (context, symbolPath) {
      const origSymbols = context.CDV_origSymbols;
  ...
  define('cordova/pluginloader', function (require, exports, module) {
    ...
    function onScriptLoadingComplete(moduleList, finishPluginLoading) {
      ...
      for (var i = 0, module; (module = moduleList[i]); i++) {
        if (module.clobbers && module.clobbers.length) {
          for (let j = 0; j < module.clobbers.length; j++) {
            modulemapper.clobbers(module.id, module.clobbers[j]);
          }
        }

        if (module.merges && module.merges.length) {
          for (let k = 0; k < module.merges.length; k++) {
    ...
    exports.load = function (callback) {
      const moduleList = require('cordova/plugin_list');
  ...
  define('cordova/utils', function (require, exports, module) {
    ...
    utils.createUUID = function () {
      return `${UUIDcreatePart(4)}-${UUIDcreatePart(2)}-${UUIDcreatePart(
        2,
      )}-${UUIDcreatePart(2)}-${UUIDcreatePart(6)}`;
    };
